...
const data = [
  { type: 'scatter', x: energy, y: f1, mode: 'lines', name: "Se f'" },
  { type: 'scatter', x: energy, y: f2, mode: 'lines', name: 'Se f"' },
];

function App() {
  const elementDivs = elements.map((element) =>
    <div key={element.z}>{element.symbol}</div>
  );
  return (
    <div className="App">
      <div className="elements">{elementDivs}</div>
      <Plot data={data} />
      <form>
        <label>Wavelength / Å<input type="number"></input></label>
        <label>Energy / eV<input type="number"></input></label>
      </form>
      <table>
        <thead><tr><th>Element</th><th>F'</th><th>F"</th></tr></thead>
        <tbody>
          <tr><td>Se</td><td>-8.320</td><td>3.846</td></tr>
        </tbody>
      </table>
    </div>
  );
